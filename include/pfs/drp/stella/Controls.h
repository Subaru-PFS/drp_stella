...
struct FiberTraceFunctionFindingControl {
  ...
  LSST_CONTROL_FIELD(apertureFWHM, float, "FWHM (in pixels) of an assumed Gaussian spatial profile for tracing the spectra");
...
struct TwoDPSFControl {
    ...
    LSST_CONTROL_FIELD(xFWHM, float, "FWHM (in pixels) of an assumed Gaussian PSF perpendicular to the dispersion direction");
    LSST_CONTROL_FIELD(yFWHM, float, "FWHM (in pixels) of an assumed Gaussian PSF in the dispersion direction");
...
struct DispCorControl {
    ...
    /// The main difference between 'searchRadius' and 'maxDistance' is that
    /// searchRadius is an int while maxDistance is a float.
    /// While searchRadius is used to determine the range to fit, maxDistance
