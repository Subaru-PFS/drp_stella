...
class FunctionN : public lsst::afw::math::Function<T> {
  ...
  public:
    ...
    Array1D getParameters() const noexcept {
        return utils::vectorToArray(Function::getParameters());
    }
...
class BasePolynomialFunctionN : public FunctionN<T> {
  public:
    ...
    explicit BasePolynomialFunctionN(unsigned int dims, unsigned int order)
      : FunctionN<T>(dims, BasePolynomialFunctionN::nParametersFromOrder(dims, order)), _order(order) {}

    explicit BasePolynomialFunctionN(unsigned int dims, Array1D const& parameters)
      : FunctionN<T>(dims, parameters) {}
...
class PolynomialFunctionN : public BasePolynomialFunctionN<T> {
  ...
  private:
    ...
    void _initialize() {
        ...
        _exponents = _getExponents(this->getDimensions(), this->getOrder());
        // Sort the exponents:
        ...
        std::sort(
            _exponents.begin(),
            _exponents.end(),
            [](std::vector<unsigned int> const& a, std::vector<unsigned int> const& b) {
    ...
    // Workspace for polynomial calculation; saves re-allocating memory
