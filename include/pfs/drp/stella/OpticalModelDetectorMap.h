...
class OpticalModelDetectorMap : public DetectorMap {
  public:
    ...
    /// row -> spatial coordinate in slit
    Spline const& getSlitSpatialSpline(int fiberId) const {
        return getSpline(fiberId, ROW, SLIT_SPATIAL);
    }

    /// row -> spectral coordinate in slit
    Spline const& getSlitSpectralSpline(int fiberId) const {
        return getSpline(fiberId, ROW, SLIT_SPECTRAL);
    }
