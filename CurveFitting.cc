...
ndarray::Array<T, 1, 1> calculatePolynomial(
  ...
  if ((std::fabs(xRangeMin + 1.) > 0.00000001) || (std::fabs(xRangeMax - 1.) > 0.00000001)) {
      ...
      xRange[0] = xRangeMin;
      xRange[1] = xRangeMax;
      xNew = convertRangeToUnity(x, xRange);
  } else {
      xNew = x;
  }
...
ndarray::Array<float, 1, 1> fitPolynomial(
    ...
    bool haveMeasureErrors = false;
    ndarray::Array<float, 1, 1> coeffs = ndarray::allocate(degree + 1);
    ...
    auto measureErrors = std::make_shared<ndarray::Array<T, 1, 1>>(ndarray::allocate(1));

    int pos = -1;
    if ((pos = utils::KeyWord_Set(argKeywords, "MEASURE_ERRORS")) >= 0) {
      LOGLS_DEBUG(_log, "Reading MEASURE_ERRORS");
      measureErrors.reset();
      measureErrors = std::make_shared<ndarray::Array<T, 1, 1>>(argValues[pos]);
...
namespace {
  ...
  fitProfile1d(ndarray::ArrayRef<ImageT, 1, 1> const& data, // data
               ...
               ndarray::ArrayRef<bool, 1, 1> const& traceMask,   // 1 for points in fiberTrace
              ...
              )
  {
      ...
      if (traceMask.asEigen().sum() == 0) {
          throw LSST_EXCEPT(pexExcept::RuntimeError, "No good pixels");
      }
...
/************************************************************************************************************/
  ...
  template< typename ImageT>
std::tuple<float, // chi^2
           ndarray::Array<ImageT, 1, 1>, // extracted spectrum
  ...
  bool fitProfile2d(lsst::afw::image::MaskedImage<ImageT, lsst::afw::image::MaskPixel, ImageT> &data,
                    ...
                    ndarray::Array<bool, 2, 1> const& traceMask,      // 1 for points in fiberTrace
...
template ndarray::Array<float, 1, 1> chebyshev(
...
template
std::tuple<float, ndarray::Array<float, 1, 1>, ndarray::Array<float, 1, 1>, ndarray::Array<float, 1, 1>>
bool fitProfile2d(
    lsst::afw::image::MaskedImage<float, lsst::afw::image::MaskPixel, float> &,
