...
from .datamodel.h4Persistence import H4PersistenceBasicModel
...
class InferPersistenceConnections(
    ...
    h4PersistenceBasicModel = PrerequisiteConnection(
        name="h4PersistenceBasicModel",
        doc="H4RG detector persistence model",
        storageClass="H4PersistenceBasicModel",
        dimensions=("instrument", "arm", "spectrograph"),
        isCalibration=True,
    )
...
class InferPersistenceTask(PipelineTask):
    ...
    def run(
        self,
        h4PersistenceBasicModel: H4PersistenceBasicModel,
        pfsArm: PfsArm,
        pfsArmNext: PfsArm,
        ...
        trappedCharge : `np.ndarray`
            ...
            where ``nComponents`` is ``h4PersistenceBasicModel.n_components``.
        ...
        persistenceReleased = h4PersistenceBasicModel.releasedCharge(trappedCharge, expTime)
        ...
        qEnd, trappedCharge, persistenceReleased = h4PersistenceBasicModel.step(
            trappedCharge,
            np.maximum(fluxRate, 0.0),
            expTime,
            dt_gap=dtGap,
            new_shape=h4PersistenceBasicModel.spatialProfile.select(pfsArm.fiberId),
        )
    ...
    def runQuantum(
        ...
        h4PersistenceBasicModel = butler.get(inputRefs.h4PersistenceBasicModel)
        pfsArmNext = butler.get(pfsArmRefs[0])

        trappedCharge = np.zeros(
            shape=(*pfsArmNext.flux.shape, h4PersistenceBasicModel.n_components),
            dtype=np.float32,
        )
