...
class AdjustDetectorMapTask(FitDistortedDetectorMapTask):
    ...
    def calculateBaseResiduals(self, detectorMap, lines):
        ...
        if np.any(isTrace):
            # Calculate dx/dy in slit coordinates for trace lines.
            delta = 1.0
            measDetectorSlope = detectorMap.detectorModel.pixelsToDetector(
                lines.x[isTrace], lines.y[isTrace] + delta
            )
            measSlitSlope = detectorMap.opticsModel.detectorToSlit(
                measDetectorSlope[:, 0], measDetectorSlope[:, 1]
            )
            slope[isTrace] = (measSlitSlope[:, 0] - measSlit[:, 0][isTrace])/delta
