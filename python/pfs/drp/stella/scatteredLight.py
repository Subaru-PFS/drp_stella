...
from pfs.drp.stella import FiberTrace, FiberTraceSet, SpectrumSet, DetectorMap
...
class ScatteredLightModel:
    frac1: float = 0.05  # Fraction of the total power in the first component
    powerLaw1: float = 1.5  # Power-law index (2-D) of the first component
    soften1: float = 1.0  # Softening (pixels) of the first component
    frac2: float = 0.01  # Fraction of the total power in the second component
    ...
    def calculateImage(self, pfsArm: PfsArm, detectorMap: DetectorMap) -> Image:
        ...
        scattered = signal.convolve(model, self.makeKernel(), mode='same')
