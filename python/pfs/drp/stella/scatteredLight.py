...
class ScatteredLightData:
    ...
    frac1 = 1.0  # Fraction of the total power in the first component
    powerLaw1 = -1.5  # Power-law index (2-D) of the first component
    soften1 = 1.0  # Softening (pixels) of the first component
    frac2 = 0.2  # Fraction of the total power in the second component
    powerLaw2 = -3.0  # Power-law index (2-D) of the second component
    soften2 = 5.0  # Softening (pixels) of the second component
    halfSize = 4096  # Half-size of the kernel
...
class ScatteredLightConfig(Config):
    scale = DictField(
        keytype=str,
        itemtype=float,
        default=dict(b3=6.6e-2, r3=5.8e-2, n3=6.5e-2, m3=5.8e-2),
    ...
    def getData(self, arm: str, spectrograph: int) -> ScatteredLightData:
        ...
        return ScatteredLightData(
            frac1=self.frac1[camera],
            powerLaw1=self.powerLaw1[camera],
            soften1=self.soften1[camera],
            frac2=self.frac2[camera],
            powerLaw2=self.powerLaw2[camera],
            soften2=self.soften2[camera],
...
class ScatteredLightTask(Task):
    ...
    def run(self, image: MaskedImage, pfsArm: PfsArm, detectorMap: DetectorMap) -> Struct:
        ...
        """
        model = self.calculateModel(pfsArm, detectorMap)
        image -= model
