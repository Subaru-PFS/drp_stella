...
class ReduceArcRefSpecConfig(Config):
    ...
    radiusXCor = Field(
        doc = "Radius in pixels in which to cross correlate a spectrum relative to the reference spectrum",
        dtype = int,
        default = 50,
    )
    lengthPieces = Field(
        doc = "Length of pieces of spectrum to match to reference spectrum by stretching and shifting",
        dtype = int,
        default = 500,
    )
    docString = "Number of iterations > spectrumLength / lengthPieces, e.g. spectrum length is 3800 pixels, "
    docString += "<lengthPieces> = 500, <nCalcs> = 15: run 1: pixels 0-499, run 2: 249-749,..."
    nCalcs = Field(
        doc = docString,
        dtype = int,
        default = 15,
    )
    stretchMinLength = Field(
        doc = "Minimum length to stretched pieces to (< lengthPieces)",
        dtype = int,
        default = 460,
    )
    stretchMaxLength = Field(
        doc = "Maximum length to stretched pieces to (> lengthPieces)",
        dtype = int,
        default = 540,
    )
    nStretches = Field(
        doc = "Number of stretches between <stretchMinLength> and <stretchMaxLength>",
        dtype = int,
        default = 80,
    )
    refSpec = Field(
        doc = "reference reference spectrum including path",
        dtype = str,
        default = os.path.join(
            getPackageDir("obs_pfs"),
            "pfs/arcSpectra/refSpec_CdHgKrNeXe_red.fits",
        ),
    )
    lineList = Field(
...
class ReduceArcRefSpecTask(CmdLineTask):
    ...
    def _makeArgumentParser(cls, *args, **kwargs):
        ...
        parser.add_argument(
            "--refSpec",
            help='directory and name of reference spectrum',
        )
        parser.add_argument(
            "--lineList",
    ...
    def run(self, expRefList, butler, refSpec=None, lineList=None, immediate=True):
        ...
        for arcRef in expRefList:
            ...
            for i in range(spectrumSetFromProfile.size()):
                ...
                self.log.info('calibrating spectrum %d: xCenter = %f' %
                    (i,
                     flatFiberTraceSet.getFiberTrace(i).getFiberTraceFunction().xCenter,
                    ))
                self.log.debug('specSpec.shape = %d' % specSpec.shape)
                self.log.debug('lineListArr.shape = [%d,%d]' %
                    (lineListArr.shape[0],
                     lineListArr.shape[1],
                    ))
                ...
                result = drpStella.stretchAndCrossCorrelateSpecFF(
                    specSpec,
                    refSpecArr,
                    lineListArr,
                    dispCorControl
                )
                self.log.debug('type(result.lineList) = %s: <%s>: <%s>' %
                    (type(result.lineList),
                     type(result.lineList[0]),
                     type(result.lineList[0][0]),
                    )
                )
                self.log.debug('type(spectrumSetFromProfile.getSpectrum(i)) = %s: <%s>: <%s>' %
                    (type(spectrumSetFromProfile.getSpectrum(i)),
                     type(spectrumSetFromProfile.getSpectrum(i).getSpectrum()),
                     type(spectrumSetFromProfile.getSpectrum(i).getSpectrum()[0]),
                    )
                )
                for j in range(result.lineList.shape[0]):
                    self.log.debug('result.lineList[%d][*] = %f, %f' %
                        (j,
                         result.lineList[j][0],
                         result.lineList[j][1],
                        )
                ...
                for j in range(specSpec.shape[0]):
                    self.log.debug('spectrum %d: spec.getWavelength()[%d] = %f' %
                        (i,
                         j,
                         spec.getWavelength()[j],
                        )
                    )
            ...
            if False:
                ...
                for i in range(spectrumSetFromProfile.size()):
                    ax.plot(
                        spectrumSetFromProfile.getSpectrum(i).getWavelength(),
                        spectrumSetFromProfile.getSpectrum(i).getSpectrum(),
                        '-+',
                    )
