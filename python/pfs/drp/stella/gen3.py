...
def certifyDatasets(
    ...
    with registry.transaction():
        ...
        if not query:
            raise RuntimeError(f"Unable to find newly-ingested {calibDatasetType} datasets!")
...
def butlerImport(
    repo: str,
    directory: str,
    exportFile: str | None = None,
    transfer: str | None = None,
    clobber: bool = False,
    debugger: bool = False,
) -> None:
    """Import datasets exported with ``butler export-calibs``

    This serves as an alternative to ``butler import`` that optionally clobbers
    datasets that already exist in the butler.
    ...
    exportFile : `str` or `None`, optional
        Path to the export file (YAML) generated by ``butler export-calibs``.
        If `None`, defaults to ``export.yaml`` in the given ``directory``.
    transfer : `str` or `None`, optional
        Transfer mode to use for ingest. If not `None`, must be one of 'auto',
        'move', 'copy', 'direct', 'split', 'hardlink', 'relsymlink' or
        'symlink'.
    clobber : `bool`, optional
        If `True`, clobber existing datasets with the same UUID.
    debugger : `bool`, optional
        If `True`, drop into the debugger if an error occurs during import.
    """
    log = getLogger("pfs.butlerImport")
    butler = Butler(repo)
    if exportFile is None:
        exportFile = os.path.join(directory, "export.yaml")
    with open(exportFile, "r") as fd:
        exports = yaml.safe_load(fd)

    prune = []
    num = 0
    for ii, record in enumerate(exports["data"]):
        if "type" not in record or record["type"] != "dataset":
            continue
        datasetType = record["dataset_type"]
        run = record["run"]
        for jj, rec in enumerate(record["records"]):
            if len(rec["dataset_id"]) != 1:
                raise RuntimeError(f"Expected exactly one dataset_id for record {rec}")
            uuid = rec["dataset_id"][0]
            if len(rec["data_id"]) != 1:
                raise RuntimeError(f"Expected exactly one data_id for record {rec}")
            dataId = rec["data_id"][0]

            num += 1
            try:
                ref = butler.get_dataset(uuid)
            except Exception:
                continue
            if ref is None:
                continue
            # This dataset already exists in the butler
            if not clobber:
                log.warning("Dataset %d,%d with UUID %s already exists; but clobber=False", ii, jj, uuid)
                continue
            if ref.datasetType.name != datasetType:
                raise RuntimeError(
                    f"Dataset {ii},{jj} with UUID {uuid} already exists but has a different dataset type: "
                    f"{ref.datasetType.name} vs {datasetType}"
                )
            if ref.dataId != dataId:
                raise RuntimeError(
                    f"Dataset {ii},{jj} with UUID {uuid} already exists but has a different dataId: "
                    f"{ref.dataId} vs {dataId}"
                )
            if ref.run != run:
                raise RuntimeError(
                    f"Dataset {ii},{jj} with UUID {uuid} already exists but has a different run: "
                    f"{ref.run} vs {run}"
                )
            log.info("Dataset %d,%d with UUID %s already exists; will clobber", ii, jj, uuid)
            prune.append(ref)

    butler = Butler(repo, writeable=True)
    if clobber and prune:
        log.info("Clobbering %d existing datasets...", len(prune))
        butler.pruneDatasets(prune, unstore=True, purge=True, disassociate=True)
    log.info("Importing %d datasets...", num)

    def runImport():
        butler.import_(directory=directory, filename=exportFile, transfer=transfer, format="yaml")
    ...
    if not debugger:
        ...
        try:
            ...
            log.error("Error during import: %s", exc)
            import pdb
            pdb.post_mortem()
    log.info("Done")
