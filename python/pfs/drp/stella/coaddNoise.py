...
class CoaddNoiseConfig(CoaddSpectraConfig, pipelineConnections=CoaddNoiseConnections):
    ...
    samples = Field(dtype=int, default=10, doc="Number of samples to use")
...
class CoaddNoiseTask(CoaddSpectraTask):
    ...
    def runQuantum(
        ...
        butler.put(PfsCoadd(outputs.pfsCoaddNoise.values(), getPfsVersions()), outputRefs.pfsCoaddNoise)
    ...
    def run(self, data: Mapping[Identity, Struct]) -> Struct:
        ...
        Returns
        -------
        pfsCoaddNoise : `dict` mapping `Target` to `PfsObject`
        ...
        return Struct(pfsCoaddNoise=pfsCoaddNoise)
