...
std::pair<ndarray::Array<double, 1, 1>, ndarray::Array<double, 2, 2>> solveLeastSquaresDesignCovariance(
    ...
    if (asEigenMatrix(fisher).isZero(0.0)) {
        ndarray::Array<double, 1, 1> solution = ndarray::allocate(numParams);
        ndarray::Array<double, 2, 2> covariance = ndarray::allocate(numParams, numParams);
        solution.deep() = 0.0;
        covariance.deep() = 0.0;
        return std::make_pair(solution, covariance);
    }
    ...
    return std::make_pair(copy(lsq.getSolution()), copy(lsq.getCovariance()));
}
