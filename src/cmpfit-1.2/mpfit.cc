...
#include <math.h>
#include <string.h>
#include "pfs/drp/stella/cmpfit-1.2/mpfit.h"
...
//    #endif
 ...
 OUTER_LOOP:
  ...
  #ifdef __DEBUG_MPFIT__
    printf("mpfit: iflag from jacobian matrix = %d\n",iflag);
  #endif
  if (iflag < 0) {
    goto CLEANUP;
  }
  ...
  if (qanylim) {
	...
	for (i=0; i<m; i++, ij++) {
	  sum += fvec[i] * fjac[ij];
          #ifdef __DEBUG_MPFIT__
            printf("mpfit: i=%d: fvec[%d] = %10.4g, fjac[%d] = %10.4g\n",i,i,fvec[i],ij,fjac[ij]);
          #endif
        }
        #ifdef __DEBUG_MPFIT__
          printf("mpfit: j=%d: sum = %10.4g\n",j, sum);
        #endif
      }
      if (lpegged && (sum > 0)) {
	ij = j*ldfjac;
  ...
  #ifdef __DEBUG_MPFIT__
    printf("mpfit: mp_qrfac finished\n");
  #endif
  ...
  if (iter == 1) {
    if (conf.douserscale == 0) {
      for (j=0; j<nfree; j++) {
        #ifdef __DEBUG_MPFIT__
          printf("mpfit: iter == 1: wa2[%d] = %10.4g\n",j, wa2[j]);
        #endif
        diag[ifree[j]] = wa2[j];
	if (wa2[j] == zero ) {
	  diag[ifree[j]] = one;
	...
	#ifdef __DEBUG_MPFIT__
	  printf("mpfit: iter == 1: diag[%d] = %10.4g\n",ifree[j], diag[ifree[j]]);
        #endif
      }
    }
    ...
    for (j=0; j<nfree; j++ ) {
      ...
      #ifdef __DEBUG_MPFIT__
        printf("mpfit: wa3[%d] = %10.4g\n",j, wa3[j]);
      #endif
    }

    xnorm = mp_enorm(nfree, wa3);
    #ifdef __DEBUG_MPFIT__
      printf("mpfit: xnorm = %10.4g\n",xnorm);
    #endif
    delta = conf.stepfactor*xnorm;
    #ifdef __DEBUG_MPFIT__
      printf("mpfit: delta = %10.4g\n",delta);
    #endif
  ...
  if (fnorm != zero) {
	...
	gnorm = mp_dmax1(gnorm,fabs(sum/wa2[l]));
        #ifdef __DEBUG_MPFIT__
          printf("mpfit: mp_dmax1 finished: gnorm = %10.4g\n",gnorm);
        #endif
      }
      jj += m;
    }
  ...
  if (conf.douserscale == 0) {
    for (j=0; j<nfree; j++ ) {
      ...
      #ifdef __DEBUG_MPFIT__
        printf("mpfit: mp_dmax1 finished: diag[%d] = %10.4g\n",ifree[j], diag[ifree[j]]);
      #endif
    }
  }
 ...
 L200:
  ...
  #ifdef __DEBUG_MPFIT__
    printf("mpfit: mp_lmpar finished\n");
  #endif
  /*
  ...
  #ifdef __DEBUG_MPFIT__
    printf("mpfit: mp_enorm finished: pnorm = %10.4g\n",pnorm);
  #endif
  ...
  if (iter == 1) {
    ...
    #ifdef __DEBUG_MPFIT__
      printf("mpfit: iter==1: delta = %10.4g\n",delta);
    #endif
  }
  ...
  #ifdef __DEBUG_MPFIT__
    printf("mpfit: mp_call finished: iflag = %d\n",iflag);
  #endif
  nfev += 1;
  ...
  #ifdef __DEBUG_MPFIT__
    printf("mpfit: mp_enorm finished: fnorm1 = %10.4g\n",fnorm1);
  #endif
  ...
  if (ratio <= p25) {
    ...
    #ifdef __DEBUG_MPFIT__
      printf("mpfit: mp_dmin1 finished: delta = %10.4g\n",delta);
    #endif
    par = par/temp;
  } else {
  ...
  if (ratio >= p0001) {
    ...
    #ifdef __DEBUG_MPFIT__
      printf("mpfit: mp_enorm finished: xnorm = %10.4g\n",xnorm);
    #endif
    fnorm = fnorm1;
    iter += 1;
  }
 ...
 L300:
  ...
  if (result && (result->covar || result->xerror)) {
    ...
    #ifdef __DEBUG_MPFIT__
      printf("mpfit: mp_covar finished\n");
    #endif
    if (result->covar) {
    ...
    if (result->xerror) {
      #ifdef __DEBUG_MPFIT__
        printf("mpfit: result->xerror = true\n");
      #endif
...
*/
  ...
  for (j=0; j<n; j++) {
    ...
    #ifdef __DEBUG_MPFIT__
      printf("mpfit::mp_qrfac: acnorm[%d] = %10.4g\n",j, acnorm[j]);
    #endif
    rdiag[j] = acnorm[j];
    wa[j] = rdiag[j];
    if (pivot != 0)
	...
	for (k=jp1; k<n; k++)
	  {
		...
		if ((p05*temp*temp) <= MP_MACHEP0)
	      ...
	      #ifdef __DEBUG_MPFIT__
	      printf("mpfit::mp_qrfac: wa[%d] = %10.4g\n",k, wa[k]);
              #endif
          }
      }
