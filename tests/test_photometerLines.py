...
class PhotometerLinesTestCase(lsst.utils.tests.TestCase):
    ...
    def testPhotometry(self, numLines, rtol):
        ...
        expectFiberId = sum(([ff]*numLines for ff in synthConfig.fiberId), [])
        self.assertFloatsEqual(lines.fiberId, expectFiberId)
        expectWavelength = np.concatenate([referenceLines.wavelength for _ in range(synthConfig.numFibers)])
