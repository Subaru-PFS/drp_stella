...
class PolynomialDetectorMapTestCase(lsst.utils.tests.TestCase):
    def setUp(self):
    ...
    def makePolynomialDetectorMap(self, likeBase):
        """Construct a `PolynomialDetectorMap`
        ...
        Returns
        -------
        detMap : `pfs.drp.stella.PolynomialDetectorMap`
        ...
        return PolynomialDetectorMap(base, distortion, visitInfo, metadata)
    ...
    def assertPolynomialDetectorMapsEqual(self, lhs, rhs):
        """Assert that the ``PolynomialDetectorMap``s are the same"""
    ...
    def testBasic(self):
        ...
        detMap = self.makePolynomialDetectorMap(True)
    ...
    def testReadWriteFits(self):
        ...
        detMap = self.makePolynomialDetectorMap(False)
        with lsst.utils.tests.getTempFilePath(".fits") as filename:
            detMap.writeFits(filename)
            copy = PolynomialDetectorMap.readFits(filename)
    ...
    def testFit(self):
        ...
        self.assertFloatsAlmostEqual(detMap.opticsModel.distortions[-1].getCoefficients(), 0.0, atol=1.0e-6)
