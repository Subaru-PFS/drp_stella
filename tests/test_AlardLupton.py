...
class AlardLuptonTestCase(lsst.utils.tests.TestCase):
    ...
    def testKernelErrors(self):
        """Parameter errors should be finite and informative for good fits, and track how
        well-determined a region's fit actually is"""
        trueKernel = self.makeKernel(1.2)
        trueBackground = 12.34
        ...
        solution = result.solutions[0]
        self.assertTrue(solution.success)
        self.assertTrue(np.all(np.isfinite(solution.kernelError)))
        self.assertTrue(np.all(solution.kernelError > 0.0))
        self.assertTrue(np.all(np.isfinite(solution.backgroundError)))
        self.assertTrue(np.all(solution.backgroundError > 0.0))
        self.assertTrue(np.isfinite(solution.kernelSumError))
        self.assertGreater(solution.kernelSumError, 0.0)
        # A well-constrained fit with many pixels should have errors much smaller than the signal.
        self.assertLess(np.max(solution.kernelError), 0.1 * np.max(np.abs(trueKernel)))

        # A region with no point sources (only flat background + noise) leaves the overall
        # normalization degenerate with the background level (a flat source's convolution is
        # indistinguishable from a shift in background), so its kernel-sum and background errors
        # should be markedly larger than the well-constrained region's, even though the individual
        # kernel-tap errors (which are partly protected by SVD thresholding of near-degenerate
        # directions) differ more modestly.
        kernelA = self.makeKernel(0.9)
        kernelB = self.makeKernel(1.8)
        half = self.width // 2
        lowSignalShape = (self.height, self.width - half)
        ...
        wellConstrained, lowSignal = unconstrained.solutions
        self.assertTrue(wellConstrained.success and lowSignal.success)
        self.assertGreater(np.max(lowSignal.kernelError), np.max(wellConstrained.kernelError))
        self.assertGreater(lowSignal.kernelSumError, 0.0)
        self.assertGreater(wellConstrained.kernelSumError, 0.0)
        self.assertGreater(lowSignal.kernelSumError, 10 * wellConstrained.kernelSumError)
        self.assertGreater(np.max(lowSignal.backgroundError), 10 * np.max(wellConstrained.backgroundError))

        # Under the common-kernel-sum constraint, the sum is fixed by construction, so its error
        # should be reported as exactly zero for every successfully-constrained region.
        constrained = fitAlardLuptonKernel(
            source,
            target,
            kernelHalfWidth=self.kernelHalfWidth,
        ...
        for constrainedSolution in constrained.solutions:
            self.assertTrue(constrainedSolution.success)
            self.assertEqual(constrainedSolution.kernelSumError, 0.0)

        # A region that fails to fit at all should report NaN for all three error quantities.
        ...
        self.assertTrue(np.all(np.isnan(impossible.kernelError)))
        self.assertTrue(np.all(np.isnan(impossible.backgroundError)))
        self.assertTrue(np.isnan(impossible.kernelSumError))
